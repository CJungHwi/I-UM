...
function mapRow(r: Record<string, unknown>): ConsultRow {
    return {
        ...
        channelDetail: toStrOrNull(r.channelDetail ?? r.channel_detail),
        ...
        consultSchool: toStrOrNull(r.consultSchool ?? r.consult_school),
        subject: toStrOrNull(r.subject),
        preferSchedule: toStrOrNull(r.preferSchedule ?? r.prefer_schedule),
        priorAcademyNote: toStrOrNull(r.priorAcademyNote ?? r.prior_academy_note),
        withdrawReason: toStrOrNull(r.withdrawReason ?? r.withdraw_reason),
        subjectInterestNote: toStrOrNull(r.subjectInterestNote ?? r.subject_interest_note),
        parentEducationView: toStrOrNull(r.parentEducationView ?? r.parent_education_view),
        childPersonalityNote: toStrOrNull(r.childPersonalityNote ?? r.child_personality_note),
        specialRequests: toStrOrNull(r.specialRequests ?? r.special_requests),
        notRegisteredReason: toStrOrNull(r.notRegisteredReason ?? r.not_registered_reason),
        ...
        nextContactAt: toStrOrNull(r.nextContactAt ?? r.next_contact_at),
...
export async function createConsultation(
    ...
    try {
        const rows = await callProcedure<Record<string, unknown>>(
            ...
            toStrOrNull(input.channelDetail),
            toStrOrNull(input.consultSchool),
            toStrOrNull(input.priorAcademyNote),
            toStrOrNull(input.withdrawReason),
            toStrOrNull(input.subjectInterestNote),
            toStrOrNull(input.parentEducationView),
            toStrOrNull(input.childPersonalityNote),
            toStrOrNull(input.specialRequests),
            toStrOrNull(input.notRegisteredReason),
            toMySQLDateTime(input.nextContactAt),
...
async function ensureCanMutate(
    id: number,
    gate: { role: IumUserRole; userAcademyId: number | null },
): Promise<ServerActionResult<ConsultDetail>> {
